...
export const debugDeleteFirstProduct = async (req: Request, res: Response) => {
  try {
    ...
    // fetch first product
    const { data: first, error: fetchErr } = await supabase.from('products').select('id, name_en').limit(1).single();
    if (fetchErr) {
    ...
    const { error: delErr } = await supabase.from('products').delete().eq('id', first.id);
    if (delErr) {
      return res.status(400).json({ success: false, error: delErr.message });
    }

    return res.json({ success: true, message: 'Deleted product', id: first.id, name: first.name_en });
  } catch (e) {
