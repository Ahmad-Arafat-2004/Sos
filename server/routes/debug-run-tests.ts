...
export const debugRunTests = async (req: Request, res: Response) => {
  try {
    ...
    for (const c of categories) {
      ...
      results.push({
        action: "ensure_category",
        slug: c.slug,
        ok: !catErr,
        error: catErr ? catErr.message : undefined,
        data: cat,
      });
    }
    ...
    const coffeeCat = (catsData || []).find(
      (r: any) => r.slug === "coffee-tea",
    );
    if (!coffeeCat)
      return res
        .status(500)
        .json({ success: false, error: "Category missing after upsert" });
    ...
    try {
      ...
      if (
        sampleRow &&
        Object.prototype.hasOwnProperty.call(sampleRow, "slug")
      ) {
    ...
    results.push({
      ...
      ok: !createErr,
      error: createErr ? createErr.message : undefined,
      data: createdProd,
    });

    if (createErr || !createdProd)
      return res
        .status(500)
        .json({
          success: false,
          error: "Failed to create debug product",
          details: createErr ? createErr.message : null,
          results,
        });
    ...
    const { data: updatedProd, error: updateErr } = await supabase
      ...
      .update({
        price: newPrice,
        description_en: "Updated by debug",
        description_ar: "تم التحديث بواسطة debug",
      })
    ...
    results.push({
      ...
      ok: !updateErr,
      error: updateErr ? updateErr.message : undefined,
      data: updatedProd,
    });
    ...
    results.push({
      ...
      ok: !verifyErr && verify && verify.price === newPrice,
      error: verifyErr ? verifyErr.message : undefined,
      data: verify,
    });
    ...
    results.push({
      ...
      ok: !delErr,
      error: delErr ? delErr.message : undefined,
    });
    ...
    if (dairy) {
      ...
      results.push({
        ...
        ok: !updCatErr,
        error: updCatErr ? updCatErr.message : undefined,
        data: updatedCat,
      });
    }
