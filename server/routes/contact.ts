    ...
    if (SENDGRID_KEY) {
      ...
      const body = {
        personalizations: [
          {
            to: [{ email: RECIPIENT }],
            subject: payload.subject || `New contact message from ${payload.name}`
          }
        ],
        from: { email: payload.email },
        content: [
          { type: 'text/plain', value: `Name: ${payload.name}\nEmail: ${payload.email}\nPhone: ${payload.phone || '-'}\n\nMessage:\n${payload.message}` }
        ]
      };
      ...
      try {
        ...
        if (!resp.ok) {
          ...
          addContactMessage({ ...payload, sent_via: 'sendgrid', sendgrid_status: resp.status, sendgrid_body: text });
          return res.status(200).json({ success: true, message: 'Stored locally; sendgrid returned error', details: text });
        }

        // store a copy locally as well
        addContactMessage({ ...payload, sent_via: 'sendgrid' });
        return res.json({ success: true, message: 'Message sent via SendGrid' });
      } catch (err: any) {
        // on error store
        addContactMessage({ ...payload, sent_via: 'sendgrid', error: String(err) });
        return res.status(500).json({ success: false, error: 'Failed to send via SendGrid', details: String(err) });
      }
    }
