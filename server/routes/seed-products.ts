...
export const seedProducts = async (req: Request, res: Response) => {
  try {
    ...
    const { data: catsData, error: catsFetchErr } = await supabase
      .from("categories")
      .select("*");
    if (catsFetchErr)
      return res
        .status(400)
        .json({ success: false, error: catsFetchErr.message });
    ...
    for (const p of products) {
      ...
      if (!category) {
        return res
          ...
          .json({
            success: false,
            error: `Missing category ${p.category_slug}`,
          });
      }
      ...
      try {
        ...
        if (!existing) {
          ...
          if (prodErr) {
            ...
            return res
              .status(400)
              .json({ success: false, error: prodErr.message });
          }
        }
      } catch (e) {
        ...
        return res
          .status(500)
          .json({ success: false, error: "Failed seeding products" });
      }
    }
