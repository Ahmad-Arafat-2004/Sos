...
const AdminDashboard: React.FC = () => {
  ...
  const { products: rawProducts, orders, addProduct, updateProduct, deleteProduct, updateOrderStatus, getProductStats } = useAdmin();
  ...
  React.useEffect(() => {
    if (editingProduct && (!editingProduct.store || !editingProduct.name || !editingProduct.id)) {
  ...
  const handleUpdateProduct = () => {
    if (editingProduct && editingProduct.name?.en && editingProduct.name?.ar && editingProduct.price > 0) {
  ...
  const handleDeleteProduct = (id: string) => {
    if (confirm(language === 'ar' ? 'هل أنت متأكد من ح��ف هذا المنتج؟' : 'Are you sure you want to delete this product?')) {
      deleteProduct(id);
    }
  };
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="flex space-x-8 rtl:space-x-reverse border-b">
            {[
              ...
              { key: 'products', label: language === 'ar' ? 'المنتجات' : 'Products', icon: Package },
              { key: 'orders', label: language === 'ar' ? 'الطلبات' : 'Orders', icon: ShoppingCart }
            ].map((tab) => (
              <button
                key={tab.key}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="bg-white rounded-2xl shadow-soft p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-gray-600 text-sm font-medium">
                      {language === 'ar' ? 'طلبات ��يد ا��انتظار' : 'Pending Orders'}
                    </p>
                    <p className="text-3xl font-bold text-gray-900 mt-2">
                      {stats.pendingOrders}
                    </p>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-200">
                      <th className={`py-3 text-sm font-medium text-gray-600 ${isRTL ? 'text-right' : 'text-left'}`}>
                  ...
                  <tbody>
                    {orders.slice(0, 5).map((order) => (
                      <tr key={order.id} className="border-b border-gray-100">
                        <td className="py-4 text-sm text-gray-900">#{order.id.slice(-6)}</td>
                        <td className="py-4 text-sm text-gray-900">{order.customerName}</td>
              ...
              <div className="bg-white rounded-2xl shadow-soft p-6 border-2 border-blue-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    >
                      <option value="dairy">{language === 'ar' ? 'منتجات الألبان' : 'Dairy'}</option>
                      <option value="spices">{language === 'ar' ? 'التوابل' : 'Spices'}</option>
                      <option value="oils">{language === 'ar' ? 'الزيوت' : 'Oils'}</option>
                      <option value="beverages">{language === 'ar' ? 'المشروبات' : 'Beverages'}</option>
                    </select>
                  </div>
                  <div>
                    <Label>{language === 'ar' ? 'رابط الص��رة' : 'Image URL'}</Label>
                    <Input
            ...
            {showAddProduct && !editingProduct && (
              <div className="bg-white rounded-2xl shadow-soft p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <Label>{language === 'ar' ? 'الاسم (عرب��)' : 'Name (Arabic)'}</Label>
                    <Input
                      value={newProduct.name.ar}
                      onChange={(e) => setNewProduct({...newProduct, name: {...newProduct.name, ar: e.target.value}})}
                      placeholder="اسم المنتج بالعربية"
                    />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setNewProduct({...newProduct, description: {...newProduct.description, ar: e.target.value}})}
                      placeholder="وصف المنتج بالعربية"
                    />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setNewProduct({...newProduct, store: e.target.value as Product['store']})}
                      className="w-full h-10 px-3 border border-gray-200 rounded-md"
                    >
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setEditingProduct({...editingProduct!, store: e.target.value as Product['store']})}
                      className="w-full h-10 px-3 border border-gray-200 rounded-md"
                    >
                      <option value="irth-biladi">{language === 'ar' ? 'إرث بلادي' : 'Irth Biladi'}</option>
                      <option value="cilka">Cilka</option>
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className={`py-4 px-6 text-sm font-medium text-gray-600 ${isRTL ? 'text-right' : 'text-left'}`}>
                      ...
                      <tr key={product.id} className="border-b border-gray-100">
                        <td className="py-4 px-6">
                          <div className="flex items-center space-x-3 rtl:space-x-reverse">
                            ...
                            />
                            <div>
                              <p className="font-medium text-gray-900">{product.name?.[language] || 'Unknown Product'}</p>
                              <p className="text-sm text-gray-500">{product.weight || 'N/A'}</p>
                            </div>
                          </div>
                        </td>
                        <td className="py-4 px-6 text-sm text-gray-600 capitalize">{product.category || 'unknown'}</td>
                        <td className="py-4 px-6">
                        ...
                        <td className="py-4 px-6 text-sm font-medium text-gray-900">${(product.price || 0).toFixed(2)}</td>
                        <td className="py-4 px-6">
