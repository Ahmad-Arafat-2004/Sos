  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-xl shadow-lg max-w-md w-full mx-4 p-6">
        ...
        <h3 className="text-lg font-semibold mb-4">{t('Checkout','إتمام الطلب')}</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="flex gap-3 pt-2">
            <Button type="button" variant="outline" onClick={onClose} className="flex-1">{t('Cancel','الغاء')}</Button>
            <Button type="submit" className="flex-1" disabled={loading}>{loading ? (isAR ? 'جارٍ...' : 'Processing...') : t('Send via WhatsApp and Save Order','ارسال عبر واتساب وحفظ الطلب')}</Button>
          </div>
        </form>
      </div>
    </div>
